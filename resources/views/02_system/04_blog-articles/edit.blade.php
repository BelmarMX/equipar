...
@section('title', 'Edición de categoría para el blog')
...
<div id="NavigationBar" class="uk-margin-top" uk-grid>
    ...
    <div class="uk-width-expand uk-text-center">
        <h1 class="uk-heading-primary">Edición de categoría para el blog</h1>
    </div>
...
<div class="uk-container uk-margin-top">
    {{-- Form --}}
    <form class="in uk-form-horizontal uk-margin-large" enctype="multipart/form-data" method="POST" action="{{ route('blog-articles.update', $entry -> id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="uk-margin uk-text-center">
            ...
            <div class="uk-form-controls">
                <img src="{{ url('storage/'.env('ARTICLE_FOLDER').'/' . $entry -> image) }}" width="100%">
            </div>
        </div>
        <div class="uk-margin">
        ...
        <div class="uk-margin">
            ...
            <div class="uk-form-controls">
                <input class="uk-input" type="date" name="publish" value="{{ old('publish') ?? Carbon::parse($entry -> publish) -> format('Y-m-d') }}" min={{ date('Y-m-d') }} placeholder="Título" required>
            </div>
        </div>
        <div class="uk-margin">
            <label class="uk-form-label" for="form-horizontal-text">Imagen</label>
        ...
        <div class="uk-margin">
            ...
            <div class="uk-form-controls uk-form-controls-text">
                <textarea class="uk-textarea" rows="5" name="shortdesc" placeholder="Textarea" required>{{ old('shordesc') ?? $entry -> shortdesc }}</textarea>
            </div>
        </div>
        <div class="uk-margin">
            <div class="uk-form-label">Contenido</div>
            <div class="uk-form-controls uk-form-controls-text">
                <textarea id="RichEdit" name="content">{{ old('content') ?? $entry -> content }}</textarea>
            </div>
        </div>
