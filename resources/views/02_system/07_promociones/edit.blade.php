...
@section('title', 'Edición de portafolio')
...
<div class="uk-container uk-margin-top">
    {{-- Form --}}
    <form class="in uk-form-horizontal uk-margin-large" enctype="multipart/form-data" method="POST" action="{{ route('promociones.update', $entry -> id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="uk-margin">
            ...
            <div class="uk-form-controls">
                <input class="uk-input" type="date" name="start" min="{{ date('Y-m-d') }}" value="{{ old('start') ?? $entry -> start }}" placeholder="Fecha de inicio" required>
            </div>
        </div>
        <div class="uk-margin">
            <label class="uk-form-label" for="form-horizontal-text">Fecha de finalización</label>
            <div class="uk-form-controls">
                <input class="uk-input" type="date" name="end" min="{{ date('Y-m-d') }}" value="{{ old('end') ?? $entry -> end }}" placeholder="Fecha de finalización" required>
            </div>
        </div>
        <div class="uk-margin">
            ...
            <div class="uk-form-controls">
                <div uk-form-custom="target: > * > span:first-child" class="width-100">
                    <select name="general_disc" required>
                        <option value="0" {!! $entry -> general_disc == 0 ? 'selected' : '' !!}>Promoción individual</option>
                        <option value="1" {!! $entry -> general_disc == 1 ? 'selected' : '' !!}>Promoción general</option>
                    </select>
        ...
        <div class="uk-margin">
            ...
            <div class="uk-form-controls">
                <input class="uk-input decimal" type="text" name="amount" placeholder="Monto o porcentaje (Solo si la promoción es general)" value="{{ old('amount') ?? $entry -> amount }}" required>
            </div>
            <div class="uk-form-controls">
                $<input type="radio" name="type" value="$" {!! $entry -> type == '$' ? 'checked' : '' !!} required> ó
                <input type="radio" name="type" value="%" {!! $entry -> type == '%' ? 'checked' : '' !!} required> %
            </div>
        </div>
        <div class="uk-margin">
